...
<head>
  ...
  <title>Quizz Addict</title>
  <link rel="stylesheet" href="./assets/styles/output.css">
  <link rel="shortcut icon" type="image/png" href="./assets/imgs/quizz.png"/>
  <style>
...
<body class="bg-[#F4F1DE]">
  ...
  <div class="flex flex-col gap-25">
    ...
    <form class="flex flex-col gap-7 items-center" action="../process/login.php" method="post">

      <h1 class="font-[Roboto] text-black text-5xl mb-8">Login</h1>

      <?php if (isset($_GET['registration-success'])) { ?>
        <div class="bg-green-500/10 p-4 mb-4 rounded border border-green-500 w-9/10 sm:w-7/10 lg:w-4/10 xl:w-3/10 2xl:w-4/20 text-center">
          <p class="text-green-500">Inscription réussi! Veuillez vous connecter.</p>
        </div>
      <?php } ?>

      <?php if(isset($_GET['invalid-credentials'])) { ?>
        <div class="bg-red-500/10 p-4 mb-4 rounded border border-red-500 w-9/10 sm:w-7/10 lg:w-4/10 xl:w-3/10 2xl:w-4/20 text-center">
          ...
          <p class="text-red-500">Pseudo ou mot de passe incorrect. Veuillez recommencer.</p>
      ...
      <button class="bg-[#FF9A76] w-6/10 shadow py-3  rounded-xl sm:w-4/10 lg:w-2/10 xl:w-3/20 mb-8" type="submit">
        <h2 class="text-[18px] font-[Roboto] font-medium">Connexion</h2>
      </button>

      <div class="w-9/10 h-[0.5px] bg-[#5D5945]/40 lg:w-4/10 2xl:w-3/10"></div>
      <a href="./register.php">
        <h3 class="font-[Roboto] text-[20px] font-extralight">Register</h3>
      </a>
